...
void Rotate::transform(IShape* _shape, unsigned int _fps_count)
{
    ...
    if(rule_idx != -1)
    {
        ...
        if(num_points == 0) // circle
        {
            ...
            if(mode[rule_idx] == ROTATE_FIXED)
            {
                ...
                _circle->x0 = round(relative_x * cos(rad_angle) + relative_y * sin(rad_angle)) + origin_x[rule_idx];
                _circle->y0 = round(relative_x * -1 * sin(rad_angle) + relative_y * cos(rad_angle)) + origin_y[rule_idx];
            }
